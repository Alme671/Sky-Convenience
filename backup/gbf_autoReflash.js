...
(function () {
    ...
    const DEFAULT_CONFIG = {
        ...
        autoClickMinDelay: 900,        // 自动点击最小延迟 (ms)
        autoClickMaxDelay: 1600,       // 自动点击最大延迟 (ms)
    ...
    */
    ...
    /**
     * 随机判定“人类走神事件” (Distraction Rate: 4%)
     * 用于在计算常规延迟时，偶尔加入1.5s~3s的超长延迟，打破绝对的机器节奏。
     */
    function simulateDistraction(baseDelay) {
        const DISTRACTION_RATE = 0.04; // 4% 几率走神
        if (Math.random() < DISTRACTION_RATE) {
            const extraDistractionDelay = Math.floor(Math.random() * 1500) + 1500;
            Logger.debug(`👀 模拟人类走神，增加额外延迟: ${extraDistractionDelay}ms`);
    ...
    function getRandomDelay() {
        const min = Math.max(0, config.minDelay);
        const max = Math.max(min, config.maxDelay);
        const baseDelay = getGaussianRandom(min, max, 1.2); // 稍微偏向更小的延迟一点
        return simulateDistraction(baseDelay);
    }
    ...
    function getAutoClickDelay() {
        const min = Math.max(0, config.autoClickMinDelay);
        const max = Math.max(min, config.autoClickMaxDelay);
        return getGaussianRandom(min, max, 1);
    }

    /**
     * 模拟人类点击（拆分调用栈 + 高斯分布偏移，纯脚本自驱动）
     */
    function simulateHumanClick() {
        // 利用 setTimeout 切断原先由网络请求/定时器触发的方法调用栈
        // 避免安全插件检查 error.stack 发现异常源
        setTimeout(() => {
            try {
                const container = document.querySelector('#wrapper') || document.querySelector('#mobage-game-container') || document.body;
                const rect = container.getBoundingClientRect();

                // 计算坐标 (中心点 + 随机微扰动)
                const baseCenterX = rect.left + rect.width * 0.5;
                const baseCenterY = rect.top + rect.height * 0.4; // 偏屏幕中上部
                const x = baseCenterX + (Math.random() - 0.5) * 80;
                const y = baseCenterY + (Math.random() - 0.5) * 60;

                const target = document.elementFromPoint(x, y) || container;
                const targetInfo = `${target.tagName}${target.id ? '#' + target.id : ''}${target.className ? '.' + String(target.className).split(' ')[0] : ''}`;

                Logger.info(`🖱️ 准备点击: 坐标(${Math.round(x)}, ${Math.round(y)}), 目标元素: ${targetInfo}`);

                const commonOpts = { bubbles: true, cancelable: true, clientX: x, clientY: y, screenX: x, screenY: y };
                let i = 0;

                // 每发一个事件加入 10-30ms 的微小停顿，避免机器的 0ms 同步发包
                function dispatchNextEvent() {
                    if (i === 0) target.dispatchEvent(new PointerEvent('pointerdown', { ...commonOpts, button: 0, buttons: 1, pointerId: 1, pointerType: 'mouse', isPrimary: true }));
                    else if (i === 1) target.dispatchEvent(new MouseEvent('mousedown', { ...commonOpts, button: 0, buttons: 1 }));
                    else if (i === 2) target.dispatchEvent(new PointerEvent('pointerup', { ...commonOpts, button: 0, buttons: 0, pointerId: 1, pointerType: 'mouse', isPrimary: true }));
                    else if (i === 3) target.dispatchEvent(new MouseEvent('mouseup', { ...commonOpts, button: 0, buttons: 0 }));
                    else if (i === 4) {
                        target.dispatchEvent(new MouseEvent('click', { ...commonOpts, button: 0, buttons: 0 }));
                        if (typeof target.click === 'function') target.click(); // fallback
                        Logger.info(`🖱️ 自动点击完成 ✅`);
                    }

                    i++;
                    if (i <= 4) {
                        setTimeout(dispatchNextEvent, Math.floor(Math.random() * 20) + 10);
                    }
                }

                // 启动异步独立事件链
                requestAnimationFrame(dispatchNextEvent);

            } catch (e) {
                Logger.error('自动点击过程出错:', e);
            }
        }, Math.floor(Math.random() * 50) + 20); // 脱离当前宏任务环境
    }
    ...
    function scheduleGoBack(matchedUrl) {
        ...
        pendingTimer = setTimeout(() => {
            pendingTimer = null;
            Logger.info('🔙 执行后退操作');
            // updateStatus('triggered', `已后退 (第 ${triggerCount} 次)`);
            window.history.back();
